...
Complete plans stay visible in `crack dashboard`, but they are not default `route_to_existing_plan` candidates. Default routing is sequential: one active incomplete plan receives follow-up requests, and multiple active incomplete plans require `--plan`. Router agent context is used only when `--router` is passed; completed plans may appear there only as conservative diagnostics such as "excluded from routing because complete." Do not treat this as dependency scheduling or automatic parallelization.
...
2. Submit the user request:
   ...
   If a PR lock exists, the request goes to `.crack/inbox.md`. If `--plan <path>` is provided, the request goes to that plan's `queue.md`. Otherwise Crack appends to the only active incomplete plan, creates a new plan when none exists, or asks for `--plan` when multiple active incomplete plans exist. Pass `--router` to let the Router agent choose between an existing plan and a new branch.
...
- `crack submit <prompt>` or `crack route <prompt>`: route a request. Options: `--plan <path>`, `--branch <name>`, `--title <title>`, `--reason <text>`, `--router`.
...
- `crack pr-check [--router]`: inspect the active PR lock. If the PR is merged, clear the lock and drain inbox requests. `--router` also enables the Router agent for the automatic drain.
- `crack drain [--router]`: reroute queued inbox requests when no PR lock is active. `--router` enables the Router agent while draining.
...
- Use `--plan` whenever more than one active incomplete plan exists. Use `--router` only when you want the Router agent to choose or create parallel work.
...
- Treat `submit` and `route` as state-mutating operations. They must be serialized so each routing choice is based on the latest active incomplete plans and queues.
