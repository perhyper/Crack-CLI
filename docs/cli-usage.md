...
새 Plan을 만들 때는 Crack이 branch 이름을 결정한다. 명시적으로 `--branch`를 주지 않으면 요청 제목을 slug로 바꿔 `codex/<slug>` 형태의 branch를 만든다.
...
- 사용자가 `--plan <path>`를 주면 해당 Plan의 `queue.md`에 요청을 append한다.
- 기본값은 Router agent를 사용하지 않는 순차 routing이다. active incomplete Plan이 하나면 그 Plan의 `queue.md`에 append한다.
- active incomplete Plan이 여러 개면 `--plan <path>`를 명시하거나 `--router`로 Router agent를 켜야 한다.
- default routing candidate가 되는 active incomplete Plan이 없으면 새 Plan을 만든다.
- `--router`를 주면 Router agent가 기존 Plan에 붙일지, 새 Plan과 branch를 만들지 결정한다.
...
Plan status도 같은 규칙으로 진단한다. `plan.md`의 commit unit 중 하나라도 `log.md`에 완료 기록이 없으면 Plan은 `active`다. 모든 commit unit이 완료 기록을 가지고 있으면 Plan은 `complete`다. `complete` Plan은 dashboard에는 계속 보이지만 기본 순차 routing이나 RouterAgent의 `route_to_existing_plan` 후보가 아니다. 대신 필요한 경우 "완료되어 routing 후보에서 제외됨" 같은 보수적인 진단 정보만 남긴다. 이 모델은 관계 힌트를 담을 자리는 두지만, dependency scheduler나 자동 병렬 실행을 도입하지 않는다.
...
`drain`은 `inbox.md` 요청을 위에서부터 하나씩 기본 순차 routing에 다시 넣고, 성공한 요청은 inbox에서 제거한다. Drain 도중 새 lock이 생기면 남은 요청은 inbox에 그대로 둔다. `crack drain --router`를 사용하면 drain 중에도 Router agent를 켠다.
...
- active incomplete plan이 하나 있으면 기본값으로 그 Plan의 `queue.md`에 요청을 추가한다.
- active incomplete plan이 여러 개 있으면 `--plan <path>`를 명시하거나 `--router`를 사용해야 한다.
- `--router`를 주면 Router agent가 기존 Plan에 붙일지 새 Plan을 만들지 판단한다.
...
- `crack pr-check --router`를 사용하면 자동 drain 중에도 Router agent를 켠다.

### `crack drain`

`.crack/inbox.md`에 쌓인 요청을 순서대로 다시 routing한다.
...
PR lock이 남아 있으면 drain하지 않고 중단한다. 기본값은 순차 routing이며, `--router`를 주면 Router agent가 각 요청을 판단한다. Drain 중 새 lock이 생기면 남은 요청은 inbox에 보존된다.
...
- 병렬 branch 판단이 필요할 때만 `submit`, `route`, `drain`, `pr-check`에 `--router`를 붙인다.
